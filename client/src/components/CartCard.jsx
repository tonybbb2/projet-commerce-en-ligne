    ...
    return (
        <div>
            <div className="flex flex-row">
                <article className="flex flex-col rounded-[1.5rem] p-[2rem] pb-[1rem] border border-secondary/20 shadow-xl transition-all duration-[1s] ease-out-expo active:scale-95">
                    <div className="flex flex-row">
                        <div className="flex flex-col">
                            <div className="grid gap-[5px]">
                                <p className="text-white text-[1.3rem] font-bold" rv-text="item.product_title | productTitle">{title}</p>
                                <p className="text-white text-[1.1rem] opacity-60" rv-text="item.product_title | productColorname">{color}</p>
                                <span className="text-white text-[1.1rem] opacity-60" rv-text="item.variant_title">{size}</span>
                            </div>
                            <span className="text-white mt-auto font-bold" rv-html="item.price | money Currency.currentCurrency">{pricing} USD</span>
                        </div>
                        <div className="ml-20">
                            <img className="lazyload-fade absolute-tl wh-full-cover rounded-[1rem] ls-is-cached lazyloaded" rv-src="item.image | product_image_size 'medium'" src={cover}></img>
                        </div>
                    </div>
